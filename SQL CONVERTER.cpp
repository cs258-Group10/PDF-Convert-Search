...
{
	//for sql query
	if(S.find("NA")==string::npos and S!="")
	{
		cout<<"to_date(\'"<<S<<"\',\'dd/mm/yyyy\'),";
	}
	else
	cout<<"\'\',";
	//assunming date of filing application contains 10 characters + just after ':'
}
...
string  perform(string start_word, string end_word,int date)
{
	start_word = purify(start_word);

	if(start_word.find(':')!=string::npos)
		print_date(start_word.substr(start_word.find(':')+1));
...
//	cout<<HEAD;
	...
	for(int i=1;get_next(S);++i)
	{
		...
		print_date(S); 		//Date of Publication of Abstract u/s 11(A)
...
int main(int argc, char *argv[])
{
	...
	while(std::getline (std::cin,S))
	{
		...
		if(S.find(start)!=string::npos)
		{
			...
			print_date(S.substr(S.find(':')+1,10));
			print_date(S.substr(S.find(": ")+2));
			...
			//--------------PRIORITY DATE-------------------------
			S=perform(S,"(33) Name of priority country",1);
